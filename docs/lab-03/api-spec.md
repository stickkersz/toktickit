...
The client normally calls `/api` through the Vite dev proxy, so the cookie is first-party and CORS is not involved. For a page served from another origin the API supports credentialed CORS (BR-62, AC-45):
