...
| API-22 | API | BR-30, api-spec.md §10 | Two concurrent `DELETE /api/attachments/:id` requests against the same active attachment | Exactly one returns 200; the other returns 409 `ALREADY_REMOVED`; the stored `removalReason` is the winner's and is never overwritten | `server/tests/lab-02/attachments.api.test.ts` | Pass |
| API-23 | API | BR-25, BR-31, api-spec.md §7 | A multi-file upload where one file's insert fails and a later file is never reached | 500 carries `uploaded`/`failed`; the committed file is reported as uploaded, the failed and never-reached files are both reported with `reason: "UPLOAD_FAILED"`, and every submitted file appears in exactly one array | `server/tests/lab-02/attachments.api.test.ts` | Pass |
...
| BR-30 (removal audit trail retained) | API-15, UI-13, API-22 |
| AC-05 | API-02, UI-05 |
| AC-06 | UI-16 |
...
|---|---|---|---|
| Server unit + API | `cd server && npm test` | Pass | 12 files, 82 tests, 0 skipped |
...
A full review of the Lab 2 diff was run before submission. Nine issues were found. The four that broke a documented rule or a graded acceptance criterion were fixed and are listed first. The rest are recorded rather than fixed at the end of the sprint, since changing more carried more regression risk than the defects themselves; each names its file so it can be picked up in Lab 3. Two of them are reachable through ordinary use and are marked as such, so this list is not a claim that the remaining issues are unreachable.
...
- `server/src/app.ts` (soft removal) — the `isRemoved` check and the update were not atomic, so two concurrent removals of the same attachment both succeeded. The second overwrote `removedAt`/`removalReason`, which BR-30 requires be retained, and answered 200 where `api-spec.md` §10 documents 409 `ALREADY_REMOVED`. That is a deviation from both the business rule and the published contract, not merely an untested edge case. The write is now conditional on the row still being active, so exactly one removal wins and the loser gets the documented 409. Covered by API-22, verified by reverting the fix and confirming the test fails.
- `server/src/app.ts` (batch upload) — an unexpected failure part-way through a multi-file upload returned a bare 500 with no `uploaded` array, so `TicketDetail` reported every file as failed even though earlier files were already committed and attached to the Ticket. BR-25 and BR-31 both require per-file reporting, so this broke the documented behaviour and invited the Requester to re-upload a file that had actually saved. The partial result now travels with the 500 and every submitted file is accounted for: the committed ones in `uploaded`, and both the file that failed and any the loop never reached in `failed` with the new `UPLOAD_FAILED` reason, so a Requester can always tell which files still need retrying. `api-spec.md` §7 documents the envelope and the new reason, and the client reconciles the result instead of discarding it. Covered by API-23.
...
- `client/src/screens/CreateTicket.tsx:93` — **reachable through ordinary use.** Removing one rejected attachment row does not re-validate the rows that remain, so selecting six valid files, then removing one, leaves the sixth showing `MAX_ATTACHMENTS_EXCEEDED` and excluded from the upload even though the Ticket now has room for it. The Requester has to remove and re-add that file to clear it.
...
- `client/src/screens/TicketDetail.tsx:106` — **reachable through ordinary use.** The client-side active-attachment count is seeded from the last rendered value, so choosing a second batch while the first upload is still in flight can let the client-side check pass when the Ticket is already at the limit, and the first batch's in-progress rows disappear from the list. The server still enforces BR-39 correctly, so the limit itself holds; the defect is in the client's feedback.
