...
- 500 `INTERNAL_ERROR`: an unexpected failure part-way through the batch. This response also carries `uploaded`/`failed` alongside `error`/`message`, the second documented departure from the plain `### Error shape` envelope. Any Attachment rows committed before the failure are real and already attached to the Ticket, so BR-25 and BR-31 require them to be reported rather than discarded; without them the client would tell the Requester that every file failed and invite a re-upload of one that actually saved. Every file in the request appears in exactly one of the two arrays: files the loop never reached, and the file that failed, are reported in `failed[]` with `reason: "UPLOAD_FAILED"`, so the Requester can always tell which of the files they chose still need retrying. Either array may be empty.
- `UPLOAD_FAILED` is a per-file `reason` that only this 500 produces. Unlike the other three reasons it is not a validation outcome and has no client-side equivalent: it means the server never reached a decision on that file, not that the file was judged and rejected.
