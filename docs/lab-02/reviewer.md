...
Every fix commit above was re-checked by the reviewer against the actual diff before approval, not accepted on the strength of the commit message alone. See for example PR #12's third round, where a fix for one line surfaced a leftover error-envelope inconsistency the previous round had introduced.
