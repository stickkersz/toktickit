...
- New dependencies required beyond the current Lab 1 stack: `multer` (server, file upload), `react-router-dom` (client, screen navigation), `@playwright/test` (installed at the repo root alongside `playwright.config.ts`, driving `e2e/lab-02` for E2E tests + responsive screenshots per handout §8.8). All within the existing Express/Vite/React/Prisma stack, no framework change.
