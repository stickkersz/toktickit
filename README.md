...
The Lab 2 Development Requester selector no longer exists: you are always the account you signed in as. Each role sees only its own navigation, and a screen its role may not use shows an "Access denied" state even by direct URL. Administrators' User Management and the Ticket Detail screens arrive in later Issues and are placeholders until then. Once an account has chosen its own password the seed never resets it, so to sign in with `ChangeMe!23` again use an account you have not touched yet.
